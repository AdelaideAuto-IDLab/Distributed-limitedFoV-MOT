function [handles,ospa_result,ospa2_result]= plot_results(model,truth,meas,est,varargin)
    try
        % --- Input Parser
        p = inputParser;
...
%         [Y_track,l_birth,l_death]= extract_tracks(est.X,est.track_list,est.total_tracks);
        ...
        [Y_track,~,~,~]= extract_tracks_with_labels(est,1,truth.K);
        ...
        if p.Results.show_plots
            ...
            %plot y estimate
            for t=1:size(Y_track,3)
                yhline2= line(1:truth.K,Y_track(3,:,t),'LineStyle','none','Marker','.','Markersize',8,'Color',colorarray.rgb(t,:));
            end
        ...
        if ~isfield(model,'ospa')
            ...
            ospa_p = model.ospa.p;
            ospa_c = model.ospa.c;
            if ~isfield(model.ospa,'win_len')
                win_len = 10;
            else
                win_len = model.ospa.win_len;
            end
        end
        for k=1:meas.K
        ...
        for i = 1:length(win_len)
            ospa2_cell{i} = compute_ospa2(X_track([1 3],:,:),Y_track([1 3],:,:),ospa_c,ospa_p,win_len);
        end
        ...
        if p.Results.show_plots
            ...
            subplot(3,1,1);
            for i = 1:length(win_len)
                plot(1:truth.K,ospa2_cell{i}(1,:),'k'); grid on; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA$^{(2)}$ Dist','interpreter','latex');
            ...
            subplot(3,1,2);
            for i = 1:length(win_len)
                plot(1:truth.K,ospa2_cell{i}(2,:),'k'); grid on; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA$^{(2)}$ Loc','interpreter','latex');
                windowlengthlabels{i} = ['$L_w = ' int2str(win_len(i)) '$'];
            end
            
            subplot(3,1,3);
            for i = 1:length(win_len)
                plot(1:truth.K,ospa2_cell{i}(3,:),'k'); grid on; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA$^{(2)}$ Card','interpreter','latex');
                windowlengthlabels{i} = ['$L_w = ' int2str(win_len(i)) '$'];
            end
        ...
        ospa_result = ospa_vals';
        ospa2_result = cell2mat(ospa2_cell);
    catch msg
        disp(['plot_results error: ',msg.message]);
        ospa_result = zeros(truth.K,3)* NaN;
        ospa2_result = zeros(truth.K,3)* NaN;
        handles = [];
    end
